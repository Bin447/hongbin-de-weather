    ...
    , ui(new Ui::Widget)
{
    ...
    setWindowTitle(tr("hongbin's weather"));
...
void Widget::slot_get_weather_info(WEATHER_INFO * info)
{
    ...
    showMessageList.append("Today is " + info->forecast[0].dataAndWeek);
    showMessageList.append(tr("The air quality today is") + info->realTime.airQuality);
    showMessageList.append(info->forecast[0].windDirAndPower);
    showMessageList.append(info->forecast[0].notice);

    if(info->forecast[0].weatherType.contains("Cloudy"))
    {
        pixmap = ":/icons/weather_icons/duoyun.png";
    }
    else if(info->forecast[0].weatherType.contains("Sunny"))
    {
